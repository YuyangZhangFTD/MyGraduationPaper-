    ...
    for f in range(para_k):
        for i_epoch in range(epoch_n):
            for k,v in para_m.items():
                ...
                q[f,k[1]] += learning_rate * (err*p[f,k[0]] + reg*q[f, k[1]])
                p[f,k[0]] += learning_rate * (err*q[f,k[1]] + reg*q[f, k[0]])
    return p, q
    ...
    for f in range(para_k):
        for i_epoch in range(epoch_n):
            for k,v in para_m.items():
                # k[0]: user
                # k[1]: item
                err = v - mu - bu[k[0]] - bi[k[1]] - q[:f+1,k[1]].T * p[:f+1,k[0]]           
...
@rt.fn_timer
def pred(para_m, para_test, para_k, with_bias=False, epoch_n=10, learning_rate=0.05, reg=0.1):
    """
    ...
    res = []
    if with_bias:
        mu, bu, bi, p, q = mf_with_bias(para_m, para_k, epoch_n=epoch_n, learning_rate=learning_rate, reg=reg)
        for i in range(len(para_test)):
            k = para_test[i]
            hat = mu + bu[k[0]] + bi[k[1]] + q[:,k[1]].T * p[:,k[0]]
            res.append([para_test[i][0], para_test[i][1], hat]) 
    else:
        p, q = mf_pq(para_m, para_k, epoch_n=epoch_n, learning_rate=learning_rate, reg=reg)
        for i in range(len(para_test)):
            k = para_test[i]
            hat = q[:,k[1]].T * p[:,k[0]]
